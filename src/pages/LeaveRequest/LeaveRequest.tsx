import { Box, Divider, Paper, Stack, Table, TableBody, TableContainer, TableRow, Typography } from '@mui/material';
import { TableBodyCell } from 'components/common/MuiTable';
import { useEffect, useMemo, useState } from 'react';
import dayjs from 'dayjs';
import 'dayjs/locale/th';
import { LeaveData, Profile, AnnualLeaveEntitlement } from 'type.global';
...
import { getAnnualLeaveEntitlement, getUsersList } from 'context/FirebaseProvider/firebaseApi/userApi';
import { summarizeUserLeave } from 'utils/summarizeUserLeave';
import { numberjs } from 'utils/numberJs';
import { AmountBox } from 'pages/HomeLanding/components/Menu/LeaveRemainingMenuBox';
            ...
            <Box marginBottom={4}>
                ...
                >
                    {userUsedLeaveDays.map((u) => (
                        <Paper key={u.suid} sx={() => ({ padding: 2, borderRadius: 2, flex: 1, boxShadow: '0 0 5px 5px #a0a0a03a' })}>
                            ...
                            {u.summaryLeaveDays.map((s) => (
                                <Box key={s.years}>
                                    ...
                                    <Stack direction={'row'} width={'100%'} spacing={1} justifyContent={'space-around'}>
                                        <AmountBox
                                            remaining={numberjs(s.all?.sick).del(s.used?.sick).value}
                                            total={s.all?.sick || 0}
                                            label='ลาป่วย'
                                            type='SICK'
                                        />
                                        <AmountBox
                                            remaining={numberjs(s.all?.personal).del(s.used?.personal).value}
                                            total={s.all?.personal || 0}
                                            label='ลากิจ'
                                            type='PERSONAL'
                                        />
                                        <AmountBox
                                            remaining={numberjs(s.all?.vacation).del(s.used?.vacation).value}
                                            total={s.all?.vacation || 0}
                                            label='ลาพักร้อน'
                                            type='VACATION'
                                        />
                                    </Stack>
